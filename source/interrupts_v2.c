...
void interrupt_handler(void)
{
  ...
  if(CAUSE_IP_GET(cause, INT_TIMER)){
	...
	** disk (INT_DISK = 3)  */
	devAddrBase =  0x1000.0050 + ((INT_DISK - 3) * 0x80) + (dnum * 0x10); //Corretto, errore di esponente
	...
	*devAddrBase + = DEV_C_ACK;
	...
	dnum = get_bit_num(get_bit_mask(*dev_bitmap));
	devAddrBase =  0x1000.0050 + ((INT_TAPE - 3) * 0x80) + (dnum * 0x10); //Corretto
	...
	*devAddrBase = DEV_C_ACK;
	...
	if(CAUSE_IP_GET(cause, INT_UNUSED)){
	  ...
	  dnum = get_bit_num(get_bit_mask(*dev_bitmap));
	  devAddrBase =  0x1000.0050 + ((INT_UNUSED - 3) * 0x80) + (dnum * 0x10); //Corretto
	  ...
	  *devAddrBase = DEV_C_ACK;
	  ...
	  if(CAUSE_IP_GET(cause, INT_PRINTER)){
	    ...
	    dnum = get_bit_num(get_bit_mask(*dev_bitmap));
	    devAddrBase =  0x1000.0050 + ((INT_PRINTER - 3) * 0x80) + (dnum * 0x10); //Corretto
	    ...
	    *devAddrBase = DEV_C_ACK;
	    ...
	    if(CAUSE_IP_GET(cause, INT_TERMINAL)){
	      ...
	      devAddrBase =  0x1000.0050 + ((INT_TERMINAL - 3) * 0x80) + (dnum * 0x10); //Corretto
	      /* read */
